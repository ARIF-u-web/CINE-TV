...
export const Route = createFileRoute("/tv")({
  head: () => ({
    meta: [
      { title: "TV Shows — CINE TV" },
      { name: "description", content: "Browse popular and top rated TV shows on CINE TV." },
    ],
  }),
  component: TVPage,
});
...
function TVPage() {
  ...
  const q = useQuery({
    queryKey: ["tv", tab],
    queryFn: () => (tab === "popular" ? tmdb.popularTV() : tmdb.topRatedTV()),
  });
  return (
    <PageLayout>
      <div className="mx-auto max-w-7xl px-4 py-10">
        <h1 className="font-display text-5xl tracking-wide">TV Shows</h1>
        ...
        <div className="mt-8">
          ...
          ) : q.data ? <MediaGrid items={q.data.results} type="tv" /> : null}
        </div>
      </div>
    </PageLayout>
  );
